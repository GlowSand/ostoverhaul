...
public class OverhaulMusicTracker implements SoundInstanceListener{
    ...
    public void tick() {
        ...
        if (this.current == null || this.current.getId() == OstOverhaul.WITHERING_ID) {
    ...
    public void playMusicOrSmthn(){
        if (this.client.player != null && !this.client.player.world.getEntitiesByClass(WitherEntity.class,this.client.player.getBoundingBox().expand(50D),null).isEmpty()){
            List<WitherEntity> witherEntities = this.client.player.world.getEntitiesByClass(WitherEntity.class,this.client.player.getBoundingBox().expand(50D),null);
            if (!witherEntities.isEmpty()) {
                if (witherEntities.get(0).getInvulnerableTimer() > 20) {
                    play(OstOverhaul.WITHER_CHARGE, 0, 1);
                }
                else{
                    play(OstOverhaul.WITHERING,0,1);
                }
            }
        }
        else if (this.client.player != null && this.timeUntilNextSong <= 0 && currentBiome != null && (this.current == null || this.current.getId() == OstOverhaul.WITHERING_ID)) {
